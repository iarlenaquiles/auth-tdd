...
describe("Authentication", () => {
  ...
  it("shouldn't authenticate with valid credentials", async () => {
    const user = await factory.create("User", { password: "123123" });

    const response = await request(app)
      .post("/sessions")
      .send({ email: user.email, password: "1222121" });
  ...
  it("shouldn't be able to access private routes without jwt token", async () => {
